...
export default function AuthScreenLink() {
	...
	return (
		<TouchableOpacity onPress={() => signInPage ? navigation.navigate('Sign Up') : navigation.navigate('Sign In')}>
			<Text style={styles.text}>{signInPage ? 'Not a member yet?' : 'Already have an account?'}</Text>
		</TouchableOpacity>
	)
}
